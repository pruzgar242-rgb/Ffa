-- RuzHub Mm2 FFA | Crimson Theme | Main Tab Only
-- Load Adonis Bypass
...
RunService.RenderStepped:Connect(function()
    ...
    Watermark.Visible = Config.SilentAim and not Config.FOVVisible and not Config.ESPVisible and not Config.HighlightVisible
    if Watermark.Visible then
        Watermark.Position = Vector2.new(playerCamera.ViewportSize.X/2 - 30, playerCamera.ViewportSize.Y/2 - 10)
    end
end)
...
local function addHitboxToPlayer(plr)
    ...
    if HitboxParts[plr] then return end -- already exists
...
player.CharacterAdded:Connect(function(char)
    ...
    -- refresh because character respawned
    if Config.HitboxExpander then
        refreshAllHitboxes()
    end
end)
...
Players.PlayerAdded:Connect(function(plr)
    plr.CharacterAdded:Connect(function(char)
        if Config.HitboxExpander and isHoldingKnife() then
            task.wait(0.5) -- wait for full character load
            addHitboxToPlayer(plr)
        end
    end)
    if Config.HitboxExpander and isHoldingKnife() then
...
MainTab:Slider({
    Title = "FOV Size",
    Description = "Radius of FOV circle",
    Min = 30,
    Max = 300,
...
MainTab:Input({
    ...
    Description = "Higher = more lead (0.1‑0.2)",
...
print("✅ RuzHub Mm2 FFA Crimson Loaded!")
print("🔴 Hitbox Expander integrated (knife only)")